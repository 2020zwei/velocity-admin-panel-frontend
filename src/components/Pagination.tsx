...
/* (Double & Single SVGs unchanged — include yours here) */
const Double = () => (
...
const Pagination = ({ totalPages, baseUrl = "/" }: PropsTypes) => {
  ...
  // detect mobile (you can change 640 to the breakpoint you prefer)
  ...
  // use smaller siblingCount on mobile to reduce number of page buttons
  const siblingCount = isMobile ? 0 : 1;

  const pages = useMemo(() => buildPages(page, totalPages, siblingCount), [page, totalPages, siblingCount]);

  const baseBtn = "flex h-8 items-center justify-center rounded-md border border-[#29293a] text-xs px-2";
  // responsive min width: mobile slightly narrower
  const baseBtnResponsive = "min-w-[2.25rem] sm:min-w-8";
  const activeBtn = "bg-[#D4368E] border-transparent text-white";
  const normalBtn = "bg-black-800 text-[#E2E2F5] hover:bg-[#181827] hover:text-white";
  const disabledBtn = "opacity-40 pointer-events-none cursor-default";

  const buildLink = (p: number) => `${baseUrl}?page=${p}`;

  return (
    // outer container ensures it won't overflow the page; allows horizontal scroll if needed
    <nav aria-label="Pagination" className="mt-5 w-full">
      <div className="w-full overflow-x-auto flex items-center justify-center"> {/* horizontal scroll on tiny screens */}
